...
bool Variant::HasKey(uint32_t key)
{
    stringstream ss;
    ss << VAR_INDEX_VALUE << key;
    return HasKey(ss.str());
}
...
uint32_t Variant::MapSize() {
    if (_type == V_NULL || _type == V_UNDEFINED)
        return 0;
    if (_type != V_TYPED_MAP && _type != V_MAP) {
        //delete by Xuefeng Li 2013-01-17 18:09
...
bool Variant::ReadJSONNumber(string &raw, Variant &result, uint32_t &start) {
    string str = "";
    //lixuefeng add 2013-08-12 22:59
    bool bDouble = false;
    /////////////////
    for (; start < raw.length(); start++)
    {
        ...
        else if (((raw[start] < '0') || (raw[start] > '9')) /* lxf add 2015-04-22 17:46*/ && ('-' != raw[start]) )
        {
            break;
        }
        str += raw[start];
